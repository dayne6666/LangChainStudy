...
query_ticket_jsonschema = {
    ...
    "properties": {
        ...
        "assigner": {"type": "string", "description": "工单分配人"},
        "status": {"type": "string", "enum": ["open", "resolved", "closed"],"description": "工单状态,open(待处理),resolved(已处理),closed(已关闭)"},
        "priority": {"type": "string", "enum": ["low", "medium", "high"],"description": "工单优先级,low(低),medium(中),high(高)"}
    },
    "required": []
}
